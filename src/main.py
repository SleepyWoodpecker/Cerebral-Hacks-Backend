...
def generate_evaluation(
    req_body: Generate_Evaluation_Body,
    response: Response,
    ...
    if authorization != os.environ.get("AUTHORIZATION_PASSWORD", None):
    ...
    all_user_evaluations_raw, chat_history = llm.query_users(
        users_dict=generated_users,
        product_dict=product,
        history=chat_history,
        image_url=product.get("Image", None),
    )
    all_user_evaluations = json.loads(all_user_evaluations_raw)
    ...
    final_evaluation_raw, chat_history = llm.generate_evaluation(history=chat_history)
    final_evaluation = json.loads(final_evaluation_raw)
    # save final copy of chat history
    db.update_chat_history(chat_id=chat_id, new_chat_history=chat_history)

    avg_star_rating = average_rating(user_output=all_user_evaluations)
    ...
    # return {
    #     "avgStarRating": 5.0,
    #     "actionBreakdown": {"purchase": 10, "like": 0, "view": 0, "total": 10},
    ...
    #     ],
    #     "bestSeason": "spring",
    #     "imageRecommendation": "sheesh the dish, today is a good day to win",
    #     "generatedReviews": [
    #         {
    #             "id": 1,
    #             "rating": 4,
